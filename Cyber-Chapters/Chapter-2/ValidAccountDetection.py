...
Script:    ValidAccountDetection.py
Purpose:   Detect brute-force attempts (4625) and suspicious
           valid account logins (4624) from Windows Event Logs.
Author:    Dr. Md Ali
Date:      2025-08-27
Version:   1.0
==========================================================
...
def DetectBruteForce():
    ...
    for account in failures:
        print("%s: %s failed logins" %
        (account,failures[account]))

def CheckDefaultAccounts():
    with open("defaults.txt","r") as f:
        defaults = [[x for x in line.split(' ')][0] for
        line in f]
        ...
        if event.StringInserts[8] == ["10","3"]:
            if event.StringInserts[5] in defaults:
                if event.StringInserts[18] not in allowed:
                    print("Unauthorized login to %s from
                    %s" %(event.StringInserts[5],event.StringInserts[18]))
